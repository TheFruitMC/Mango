...
public final class SkinnedMeshCapturingVertexConsumer implements VertexConsumer, BoneIndexReceiver {
    ...
    private static final int BONE_INDEX_BYTES = Integer.BYTES;
    static final int VERTEX_SIZE = POSITION_BYTES + UV0_BYTES + NORMAL_BYTES + BONE_INDEX_BYTES;
    ...
    private static final int BONE_INDEX_OFFSET = POSITION_BYTES + UV0_BYTES + NORMAL_BYTES;
    private static final int NO_BONE = -1;

    private ByteBuffer buffer;
    private int vertexCount;

    private int currentBoneIndex = NO_BONE;
    ...
    public SkinnedMeshCapturingVertexConsumer() {
        this.buffer = MemoryUtil.memAlloc(INITIAL_VERTEX_CAPACITY * VERTEX_SIZE);
        this.vertexCount = 0;
    }

    @Override
    public void setCurrentBoneIndex(int boneIndex) {
        this.currentBoneIndex = boneIndex;
    }
    ...
    private void writeVertex(float x, float y, float z, float u, float v, float nx, float ny, float nz) {
        ...
        MemoryUtil.memPutInt(base + BONE_INDEX_OFFSET, this.currentBoneIndex);
        this.vertexCount++;
    }
